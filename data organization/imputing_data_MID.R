...
get_imp_data <- function(merged_data) {
  ...
  merged_data_imp_bpm <- miceRanger(merged_data_bpm, m = 5, maxiter = 10, verbose = FALSE)
  saveRDS(merged_data_imp_bpm, "data/merged_data_imp_bpm_cohort.rds")
  ...
  merge_imputed_back <- function(imp_obj, original_df, vars_to_replace,
    ...
    vars_to_drop <- unique(c(vars_to_replace, covars, IVs)) # Remove DV, IV, and covariates from original data
    ...
    lapply(seq_along(imputed_list), function(i) {
      ...
      # 3. Fix outcome source
      # 04A outcome ALWAYS from original
      df <- df %>%
        left_join(
          original_df %>%
            select(participant_id, session_id, all_of(vars_to_replace)) %>%
            rename_with(~ paste0(.x, "_orig"), all_of(vars_to_replace)),
          by = c("participant_id", "session_id")
        ) %>%
        mutate(
          across(
  ...
  create_aim_datasets <- function(df, 
    ...
    join_df <- switch(data_type,
                      sleep   = df %>% filter(session_id=="ses-03A") %>%
                        select(participant_id, sleep_duration_hrs_3y = sleep_duration_hrs),
                      dep     = df %>% filter(session_id=="ses-02A") %>%
                        select(participant_id, depression_dx_y_2y = depression_dx_y),
                      obesity = df %>% filter(session_id=="ses-02A") %>%
                        select(participant_id, bmi_obesity_2y = bmi_obesity),
                      bpm = df %>% filter(session_id=="ses-03A") %>% mutate(bpm_T_bin_concern = as.numeric(mh_y_bpm_tscore  >= 65 )) %>% 
                        select(participant_id, bpm_T_bin_concern_3y = bpm_T_bin_concern, mh_y_bpm_tscore_3y = mh_y_bpm_tscore),
                      stop("data_type is not recognizable")
    )
    ...
    aim1_df <- df %>%
      ...
      mutate(across(matches("_2y$|_3y$|_y$"),
                                  ~as.numeric(as.character(.x))))
    
    # ---------- Aim2 ----------
    aim2_df <- df %>%
      filter(session_id=="ses-04A", smartphone_ownership==1) %>%
      mutate(across(matches("_y$"),
                                  ~as.numeric(as.character(.x)))) %>%
      mutate(across(all_of(smartphone_vars),
                                  ~./7, .names="{.col}_wmean_perday")) %>%
      mutate(across(ends_with("_wmean_perday"),
                                  ~case_when(.<=2~"0–2h", .<=5~"2–5h", .>5~">5h"),
                                  .names="{.col}_cat")) %>%
      mutate(across(ends_with("_cat"),
